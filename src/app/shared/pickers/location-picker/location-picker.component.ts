...
export class LocationPickerComponent implements OnInit {
  ...
  private createPlace(lat: number, lng: number) {
    ...
    this.isLoading = true;
    this.getAdress(lat, lng).pipe(switchMap(address => {
  ...
  private getMapImage(lat: number, lng: number, zoom: number) {
    ...
    &markers=color:blue%7Clabel:S%7C40.702147,-74.015794&markers=color:red%7Clabel:Place%7C${lat},${lng}
  ...
  private getAdress(lat: number, lng: number) {
   ...
   return this.http
    ...
    ${environment.googleMapsAPIKey}`).pipe(
      map(geoData => {
